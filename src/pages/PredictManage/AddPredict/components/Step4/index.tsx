...
const Step4: React.FC<Step3Props> = (props) => {
  const [form] = Form.useForm();
  const { validateFields } = form;
  const { stepData, current, currentProject, dispatch } = props;
  ...
  const [formulaInfoDeafault, setFormulaInfoDeafault] = useState<any>({});
  const [currentIndex, setCurrentIndex] = useState<any>(undefined);
  const [currentRecord, setCurrentRecord] = useState<any>({});
  const [tripDivisionObj, setTripDivisionObj] = useState<any>({});
  // if (!stepData) {
  //   return null;
  // }
  useEffect(() => {
  ...
  const handleInitTripDivision = () => {
    ...
    stepData?.step1Form?.tripDivision?.forEach((item: any) => {
      newTripDivisionObj[item[0]?.plotId] = item;
    ...
    setTripDivisionObj(newTripDivisionObj);
    setAssessTrafficTableData(newAssessTrafficTableData);
  };
  // useEffect(() => {
  //   if (currentProject) {
  //     let newAssessTrafficTableData = currentProject.plotInfo.map((item: any, index: number) => {
  //       return {
  //         num: index + 1 + '',
  //         plotId: item._id,
  //         plotCode: item.code,
  ...
  const handleOnGetParams = (type: string) => {
    ...
    if (type == 'bg') {
      ...
      newZones = stepData.step2Form.zoneInfo.filter((zone: any) => {
        if (zone?.plot || zone?.plotInfo) {
          ...
          // console.log('filterPlot', filterPlot);
          if (filterPlot.length > 0) {
            return filterPlot[0]?.buildYear == assessType ? true : false;
          }
        }
    ...
    let connectors = filterNodes.filter((item: any) => {
      if (type == 'bg') {
        let currentZoneData: any = stepData.step2Form.zoneInfo.filter(
  ...
  const handleOnSubmit = async () => {
    ...
    let flowLevel: any[] = [];
    // 背景饱和度图
    params.dmd.matrix = changeOdToDmdMatrix(bgData, bgZoneNum);
    params.resType = 0;
    ...
    // 叠加饱和度图
    params1.dmd.matrix = changeOdToDmdMatrix(spData, spZoneNum);
    params1.resType = 0;
    ...
    params2.dmd.matrix = changeOdToDmdMatrix(spData, spZoneNum);
    ...
    if (successNum == 5) {
      ...
      if (dispatch) {
        dispatch({
          type: 'predictManageAndAddPredict/submitStepForm',
          payload: {
            ...stepData,
            step4Form: {
              ...values,
  ...
  const hanldeOnCal = async () => {
    ...
    let i = 0;
    assessTrafficTableData.forEach((item: any) => {
      if (item.peakHourAttract) {
        i++;
      }
    });
    if (i < assessTrafficTableData.length) {
      message.warning('请先计算完所有的基地高峰小时交通量!');
      return;
    }
    ...
    if (assessType == '近期') {
      zoneA = [];
      zoneP = [];
      // console.log(currentProject.nearPlotProjectInfo);
    ...
    if (type == 'finish') {
      ...
      newCurrentRecord.data.forEach((item: any) => {
        if (item.personType != '商业工作人员') {
          ...
          newCurrentRecord.peakHourAttract = newCurrentRecord.peakHourAttract || 0;
          newCurrentRecord.peakHourProduct = newCurrentRecord.peakHourProduct || 0;
          newCurrentRecord.peakHourProduct =
            newCurrentRecord.peakHourProduct + carProduction + taxiProduction + '';
          newCurrentRecord.peakHourAttract =
            newCurrentRecord.peakHourAttract + carAtrraction + taxiAtrraction + '';
        }
      });
  ...
  const handleOnSetFormulaInfo = (value: any) => {};
        ...
        <Row>
          <Col span={21} offset={1} className={styles.step3Form}>
            ...
            >
              ...
              >
                <Radio.Group>
                  <Radio value="1">背景</Radio>
                  <Radio value="2">叠加</Radio>
                </Radio.Group>
              </Form.Item> */}
              <Form.Item
                label="评价年基地高峰小时交通量（单位:pcu/h）"
                name="plotTraffic"
                rules={[{ required: true, message: '请计算' }]}
              >
                <Table
              ...
              {/* <Form.Item
                label="评价年基地高峰小时交通产生量（单位:pcu/h）"
                name="assessTrafficProduction"
                rules={[{ required: true, message: '请输入' }]}
              >
                <Input placeholder="请输入" />
              </Form.Item>
              <Form.Item
                label="评价年基地高峰小时交通吸引量（单位:pcu/h）"
                name="assessTrafficAtrraction"
                rules={[{ required: true, message: '请输入' }]}
              >
                <Input placeholder="请输入" />
              </Form.Item> */}
                ...
                <Row style={{ marginBottom: '10px' }}>
                  <Col span={17} offset={7}>
                    ...
                    <table className={styles.odtable}>
                      <tbody>
                        <tr>
                          <td>{bgData.length > 0 && `${bgData[0].length}✖️${bgData[0].length}`}</td>
                          <td></td>
                          <td></td>
                          {bgData.length > 0 &&
                ...
                <Row>
                  <Col span={17} offset={7}>
                    ...
                    <table className={styles.odtable}>
                      <tbody>
                        <tr>
                          <td>{spData.length > 0 && `${spData[0].length}✖️${spData[0].length}`}</td>
                          <td></td>
                          <td></td>
                          {spData.length > 0 &&
