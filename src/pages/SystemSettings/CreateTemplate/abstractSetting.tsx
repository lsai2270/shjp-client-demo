...
const ReportManage = (props: any) => {
  ...
  const [titleType, setTitleType] = useState<any>('摘要');
  useEffect(() => {
  ...
  const hanldeOnSelected = (key: string) => {
    ...
    if (editorObj) {
      ...
      let newSectionInfo: any = sectionInfo.map((item) => {
        // 加载当前的content
        if (item._id == key) {
          editorObj.txt.html(item?.summary?.htmlContent);
        }
        // 保存上一次的content
        if (item._id == catalogueKey) {
          item.summary = { ...item.summary };
          item.summary.htmlContent = sectionHtml;
        }
        return item;
      });
  ...
  const handleOnAddElement = (eleStr: string, paramsId: any) => {
    ...
    let newsectionInfo = sectionInfo.map((item) => {
      if (eleStr.includes('tableContainer')) {
        if (item._id == catalogueKey) {
  ...
  const handleOnSave = () => {
    ...
    let newsectionInfo = sectionInfo.map((item) => {
      if (item._id == catalogueKey) {
        ...
        if (Boolean(htmlstr)) {
          item.summary = { ...item.summary };
          item.summary.content = hanldeOnResolveHtml(htmlstr);
          // console.log('item.content', item.content);
          item.summary.htmlContent = htmlstr;
      ...
      item.paramInfo.forEach((param: any, paramIndex: number) => {
        if (Boolean(param?._id)) {
          if (
            item?.content?.indexOf(param?._id) == -1 &&
            item?.summary?.content?.indexOf(param?._id) == -1
          ) {
            item.paramInfo.splice(paramIndex, 1);
          }
        } else {
          if (item?.content?.indexOf(param) == -1 && item?.summary?.content?.indexOf(param) == -1) {
            item.paramInfo.splice(paramIndex, 1);
          }
        }
      });
  ...
  const hanldeOnSetAbstract = () => {
    confirm({
      ...
      content: (
        <span>
          <span style={{ color: 'red' }}>请确认是否切换成“报告模版”进行编辑？。</span>
          <br />
        </span>
      ),
      onOk() {
        ...
        <Col span={5}>
          ...
          <div className={styles.catalogueContain}>
            {compArr[0] ? (
              ...
              <div style={{ height: '889px', minHeight: '889px' }}></div>
            )}
          </div>
          {/* 公式 */}
