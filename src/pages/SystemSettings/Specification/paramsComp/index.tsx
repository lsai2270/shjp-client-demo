...
import {
  ...
  updateDicData,
  getStandardDicData,
} from '@/services/v2/dicType';
import { getStandardList } from '@/services/v2/standard';
...
export default (props: any) => {
  ...
  const [standardData, setStandardData] = useState<any[]>([]); // 标准数据
  const [typeCodeData, setTypeCodeData] = useState<any[]>([]); // 标准参数
      ...
      let params = {
        dicData: dictionarys.map((item: any) => {
          return {
            ...
            category: 'standard',
            standardInfo: formData.standard.value,
          };
        }),
      };
  ...
  // 获取标准列表
  const handleOnGetStandard = async (params: any) => {
    const res = await getStandardList(params);
    try {
      if (res.code == 200) {
        if (res.data.data.length > 0) {
          setStandardData([...standardData, ...res.data.data]);
        }
      }
    } catch (e) {
      message.error('获取标准接口出错!');
    }
    return res.data;
  };
  const hanldeOnStandardChange = (value: any) => {
    console.log('standard', value);
    form.setFieldsValue({
      standard: value,
    });
  };
  const expandedRowRender = (record: any, index: any, indent: any, expanded: boolean) => {
    if (expanded) {
          ...
          <Col span={24}>
            <ProTable
              ...
              request={async (params, sorter, filter) => {
                const res = await getStandardDicData({ ...params, sorter, filter });
                // console.log(res);
                return {
                  data: res.data.data,
                  success: true,
            ...
            >
              ...
              >
                <Form.Item
                  ...
                  name="typeCode"
                  wrapperCol={{ span: 12 }}
                  rules={[{ required: true, message: '请选择标准参数!' }]}
                >
                  {categoryVisible && (
                ...
                >
                  <ScrollSelect
                    // pageSize={50}
                    // value={typeCodeValue}
                    handleOnGetData={handleOnGetStandard}
                    hanldeOnChange={hanldeOnStandardChange}
                    placeholder="请选择"
                  />
                </Form.Item>
                <Form.Item
