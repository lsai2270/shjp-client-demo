...
const Step3: React.FC<Step3Props> = (props) => {
  ...
  const { stepData, current, currentRecord, dispatch } = props;
  ...
  useEffect(() => {
    ...
    if (currentRecord && Object.keys(currentRecord).length > 0) {
      let initData = {};
      console.log(currentRecord.nearPlotProjectInfo);
      let resultTableData: any[] = [];
      let initTableData = currentRecord.nearPlotProjectInfo.map((item: any, index: number) => {
        initData[`name${index}`] = item.name;
        initData[`nature${index}`] = item.nature.map((n: any) => {
          return n._id;
        });
        initData[`buildArea${index}`] = item.buildArea;
        initData[`dayOfTravelRate${index}`] = item.dayOfTravelRate;
        initData[`peakHoursCoefficient${index}`] = item.peakHoursCoefficient;
        initData[`guestsCoefficient${index}`] = item.guestsCoefficient;
        initData[`trafficProductionRate${index}`] = item.trafficProductionRate;
        initData[`trafficAtrractionRate${index}`] = item.trafficAtrractionRate;
        initData[`buildYear${index}`] = { label: item.buildYear, value: item.buildYearId };
        initData[`buildEndAt${index}`] = moment(item.buildEndAt);
        tripForm.setFieldsValue(item.tripDivision[0]);
        setTripTableData(item.tripDivision);
        let dataObj = {
          _id: item._id,
          name: item.name,
          nature: item.nature,
          buildArea: item.buildArea,
          dayOfTravelRate: item.dayOfTravelRate,
          peakHoursCoefficient: item.peakHoursCoefficient,
          guestsCoefficient: item.guestsCoefficient,
          trafficProductionRate: item.trafficProductionRate,
          trafficAtrractionRate: item.trafficAtrractionRate,
          buildYear: item.buildYear,
          buildYearId: item.buildYearId,
          buildEndAt: item.buildEndAt,
          type: '2',
          tripDivision: item.tripDivision,
        };
        resultTableData.push({
          ...dataObj,
          residentTrips: item.residentTrips,
          trafficProduction: item.trafficProduction,
          trafficAtrraction: item.trafficAtrraction,
          carProduction: item.carProduction,
          carAtrraction: item.carAtrraction,
          taxiProduction: item.taxiProduction,
          taxiAtrraction: item.taxiAtrraction,
        });
        return dataObj;
      });
      form.setFieldsValue(initData);
      setTableData(initTableData);
      setResultTableData(resultTableData);
    }
  }, []);
  useEffect(() => {
  ...
  const handleOnGetParamsRatio = (data: any, index: number) => {
    ...
    // console.log("dictData['rdxcxl']",dictData['rdxcxl']);
  ...
  const handleOnDelete = (tableIndex: number) => {
    ...
    setTableData(newTableData);
    let newResultTableData = resultTableData.filter(
      (item: object, index: number) => index != tableIndex,
    );
    setResultTableData(newResultTableData);
  };
  const onPrev = () => {
