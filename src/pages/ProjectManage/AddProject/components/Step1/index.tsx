...
const Step1: React.FC<Step1Props> = (props) => {
  const { dispatch, current, stepData } = props;
  ...
  useEffect(() => {
    ...
    if (Object.keys(stepData.step1Form).length > 0) {
      ...
      // setBuildAreas(stepData.step1Form.functionalPartitioningBuildArea);
    }
    getDictDataAll();
  }, []);
        ...
        if (valueIds.indexOf(item.id) == -1) {
          ...
          item?.houseType?.forEach((item1: any) => {
            form.resetFields([
              ...
              `houseTypeNumber${item1.id}_${item1.id}`,
            ]);
          });
